...
function main()
    ...
    covB = Y' * Y / (n - 1)
    covC = [cov(X[:, i], X[:, j]) for i = 1:p, j = 1:p]
    covD = [cor(X[:, i], X[:, j]) * std(X[:, i]) * std(X[:, j]) for i = 1:p, j = 1:p]
    covE = cov(X)
    println("\nAlternative calculations of (sample) covariance matrix: ")
    @show(covA), @show(covB), @show(covC), @show(covD), @show(covE)

    ZmatA = [(X[i, j] - mean(X[:, j])) / std(X[:, j]) for i = 1:n, j = 1:p]
    ZmatB = hcat([zscore(X[:, j]) for j = 1:p]...)
    println(
    ...
    corA = covA ./ [std(X[:, i]) * std(X[:, j]) for i = 1:p, j = 1:p]
    corB = covA ./ (std(X, dims = 1)' * std(X, dims = 1))
    corC = [cor(X[:, i], X[:, j]) for i = 1:p, j = 1:p]
    corD = Z' * Z / (n - 1)
    corE = cov(Z)
    corF = cor(X)
