...
function compute_xCoords(tdArray)
...
function compute_yCoords1(tdArray)
...
@testset "end_to_end" begin
    ...
    xCoords = compute_xCoords(tdArray)
    yCoords1 = compute_yCoords1(tdArray)
    yCoords2 = compute_yCoords2()
    scatter(xCoords, yCoords1, c = :blue, label = "Calculated v", msw = 0)
    scatter!(xCoords, yCoords2, c = :red, label = "Fixed v", msw = 0)
    plot!([-10, 10], [-10, 10], c = :black, lw = 0.3, xlims = (-8,8), ylims = (-8,8), ratio = :equal, label = "", xlabel = "Theoretical t-distribution quantiles", ylabel = "Simulated t-distribution quantiles", legend = :topleft)
    end