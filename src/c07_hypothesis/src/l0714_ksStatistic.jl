...
figure(figsize = (10, 5))
subplot(121)
plt.hist(kStats1, 50, color = "b", label = "KS stat (Exponential)", histtype = "step", density = true)
plot(kGrid, [pdf(Kolmogorov(), k) for k in kGrid], "r", label = "Kolmogorov PDF")
legend(loc = "upper right")
xlim(0, 2.5)

subplot(122)
plt.hist(kStats2, 50, color = "b", label = "KS stat (Normal)", histtype = "step", density = true)
plot(kGrid, [pdf(Kolmogorov(), k) for k in kGrid], "r", label = "Kolmogorov PDF")
legend(loc = "upper right")
xlim(0, 2.5)
