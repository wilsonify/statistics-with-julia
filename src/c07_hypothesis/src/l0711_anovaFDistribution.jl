...
for i in 1:N
  mcFstatsH0[i] = anovaFStat([ rand(Normal(case1[j],stdDevs[j]),numObs[j]) for j in 1:L ])
end

mcFstatsH1 = Array{Float64}(undef, N)
for i in 1:N
  mcFstatsH1[i] = anovaFStat([ rand(Normal(case2[j],stdDevs[j]),numObs[j]) for j in 1:L ])
end
